import os
import spacy
from utils import * # Assicurati che utils.py contenga get_golden_standard, compute_metrics, compute_ndcg, compute_r_precision_at_k, precision_recall_curve_interpolated
from bench_plotting import * # Assicurati che bench_plotting.py contenga tutte le funzioni di plot menzionate
...
standard_analyzer = StandardAnalyzer() # Rinominato per chiarezza
for s in searchers.values():
    s.setSimilarity(ClassicSimilarity())

# --- Setup spaCy per la Lemmatizzazione ---
try:
    nlp = spacy.load("en_core_web_sm")
except OSError:
    print("Modello spaCy 'en_core_web_sm' non trovato. Scaricalo con: python -m spacy download en_core_web_sm")
    exit()
...
# Funzione per analizzare il testo con l'analizzatore Lucene
def analyze_with_lucene_analyzer(analyzer, text):
    token_stream = analyzer.tokenStream("field", StringReader(text))
    char_term_attr = token_stream.addAttribute(CharTermAttribute.class_)
    token_stream.reset()
    tokens = []
...
for entry in query_data:
    original_queries = entry["query"]
    fields = list(original_queries.keys())

    for model in models:
        bq_builder = BooleanQuery.Builder()
        
        # Prepara la query in base al modello
        processed_queries = {}
        for field, text in original_queries.items():
            if model == "tf_idf_std":
                # Le query per tf_idf_std vengono analizzate con StandardAnalyzer
                # Se la query contiene wildcard (*), QueryParser le gestirà direttamente.
                # Altrimenti, si analizza il testo per normalizzarlo (es. lowercase, stop words)
                if '*' not in text: # Non analizzare con standard_analyzer se ci sono wildcard, QueryParser lo fa
                    processed_queries[field] = analyze_with_lucene_analyzer(standard_analyzer, text)
                else:
                    processed_queries[field] = text
            else: # model == "tf_idf_lemma"
                # Le query per tf_idf_lemma vengono lemmatizzate
                processed_queries[field] = lemmatize_text(text)

        for field, processed_text in processed_queries.items():
            # Aggiusta la stringa della query per QueryParser per evitare errori con stringhe vuote
            if not processed_text.strip():
                # Se la stringa analizzata o lemmatizzata è vuota, salta l'aggiunta al BooleanQuery
                print(f"Attenzione: La query per il campo '{field}' nel modello '{model}' è risultata vuota dopo la pre-elaborazione. Saltata.")
                continue
            
            # Se la query originale conteneva un wildcard e il modello è tf_idf_std,
            # QueryParser gestirà il wildcard correttamente. Non serve aggiungere altre logiche qui.
            parsed = QueryParser(field, standard_analyzer).parse(processed_text) # QueryParser usa sempre StandardAnalyzer per la sua sintassi
            bq_builder.add(parsed, BooleanClause.Occur.MUST)
        
        # Se non ci sono clausole nella BooleanQuery a causa di query vuote, salta
        if bq_builder.build().clauses().isEmpty():
            print(f"Nessuna clausola valida per la query UIN {entry['uin']} nel modello {model}. Saltata l'esecuzione della ricerca.")
            continue
        ...
        if model == "tf_idf_std":
            golden = get_golden_standard(fields, original_queries, "goldstandard_vsm") # Usa le query originali per il golden standard VSM
        else:
            golden = get_golden_standard(fields, original_queries, "goldstandard_lemmatized") # Usa le query originali per il golden standard lemmatizzato

        # Assicurati che il golden standard non sia vuoto
        if not golden:
            print(f"Attenzione: Golden standard vuoto per la query UIN {entry['uin']} nel modello {model}. Le metriche potrebbero essere imprecise.")
            # Potresti voler saltare il calcolo delle metriche o assegnare 0 per tutte
            avg_curves[model].append(0.0)
            precision_dict[model].append(0.0)
            recall_dict[model].append(0.0)
            f1_dict[model].append(0.0)
            ndcg_dict[model].append(0.0)
            r_precision_dict[model].append(0.0)
            continue
...
# --- Plotting ---
output_dir = "./fake_graph_std_vs_lemma"
